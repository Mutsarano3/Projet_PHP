...
$sql = DataBase::connect()->prepare('SELECT ID_Materiel FROM materiel');
$sql->execute();
if(isset($_POST['id']))
{
    if($_SERVER['REQUEST_METHOD'] == "POST")
    {
        while($row = $sql->fetch())
        {
            if($hardware->VerifId($_POST['id'],$row['ID_Materiel']) ==  $_POST['id'])
            {
                
                $id=$hardware->IsSecure($_POST['id']);
                $query = DataBase::connect()->prepare('UPDATE materiel SET Stock = 0 WHERE ID_Materiel = ?');
            ...
            {
                echo "<p>Erreur l'ID de l'HardWare n'existe pas</p>";
...
<html>
    ...
    <a style="color:white" href="../Vue_PHP/DashBoard.php">Retour à DashBoard</a>
        <div class="container">
                ...
                <div id="form-style">
                    <h2>Veuillez rentrer un ID d'un HardWare qui est dans la BDD</h2>
